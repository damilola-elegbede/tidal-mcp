...
class TestAuthenticationFlow:
    ...
    @pytest.mark.asyncio
    async def test_full_oauth_authentication_flow(
        self, temp_session_file, mock_tidal_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_session_persistence_and_reload(
        self, temp_session_file, mock_tidal_session
    ):
...
class TestSearchIntegration:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_search_workflow(
        self, integrated_service, mock_tidal_session
    ):
        """Test complete search workflow across all content types."""
        # Mock search results
        sample_track = create_sample_tidal_track(
            123456, "Search Track", "Search Artist"
        )
        sample_album = Mock()
        sample_album.id = 789
        ...
        sample_playlist = create_sample_tidal_playlist(
...
class TestPlaylistManagementIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_playlist_lifecycle(
        self, integrated_service, mock_tidal_session
    ):
        ...
        # 1. Create playlist
        created_playlist = await integrated_service.create_playlist(
        ...
        with patch("tidal_mcp.utils.validate_tidal_id", return_value=True):
            add_success = await integrated_service.add_tracks_to_playlist(
                "new-playlist", ["123456"]
            )
        ...
        retrieved_playlist = await integrated_service.get_playlist(
            "new-playlist", include_tracks=True
        )
    ...
    @pytest.mark.asyncio
    async def test_user_playlists_management(
        self, integrated_service, mock_tidal_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_playlist_tracks_operations(
        self, integrated_service, mock_tidal_session
    ):
        ...
        with patch("tidal_mcp.utils.validate_tidal_id", return_value=True):
            # Get playlist tracks
            retrieved_tracks = await integrated_service.get_playlist_tracks(
                "test-playlist"
            )
...
class TestFavoritesIntegration:
    ...
    @pytest.mark.asyncio
    async def test_favorites_full_workflow(
        self, integrated_service, mock_tidal_session
    ):
        ...
        with patch("tidal_mcp.utils.validate_tidal_id", return_value=True):
            remove_success = await integrated_service.remove_from_favorites(
                "789", "track"
            )
            assert remove_success is True
            favorites = mock_tidal_session.user.favorites
...
class TestRecommendationsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_personalized_recommendations_flow(
        self, integrated_service, mock_tidal_session
    ):
...
class TestDetailedContentRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_complete_track_information_flow(
        self, integrated_service, mock_tidal_session
    ):
    ...
    async def test_album_with_tracks_flow(self, integrated_service, mock_tidal_session):
        ...
        with patch("tidal_mcp.utils.validate_tidal_id", return_value=True):
            # Get album details
            retrieved_album = await integrated_service.get_album(
                "456", include_tracks=True
            )
    ...
    @pytest.mark.asyncio
    async def test_artist_discography_flow(
        self, integrated_service, mock_tidal_session
    ):
...
class TestUserProfileIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_user_profile_flow(
        self, integrated_service, integrated_auth
    ):
...
class TestErrorHandlingAndRecovery:
    ...
    @pytest.mark.asyncio
    async def test_authentication_expiry_and_refresh(
        self, integrated_service, integrated_auth, mock_tidal_session
    ):
...
class TestConcurrentOperations:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_playlist_operations(
        self, integrated_service, mock_tidal_session
    ):
