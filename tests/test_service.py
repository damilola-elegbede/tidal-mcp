...
Comprehensive unit tests for the TidalService class covering search functionality,
playlist management, favorites, recommendations, and error handling.
"""
...
def sample_tidal_playlist():
    ...
    playlist = Mock()
    playlist.uuid = "playlist-uuid-123"
    playlist.id = "playlist-uuid-123"
...
class TestSearchFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_search_tracks_success(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(
                id="123456",
                title="Test Track",
    ...
    @pytest.mark.asyncio
    async def test_search_tracks_with_pagination(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(id="123456", title="Test Track", artists=[], duration=240)
            mock_convert.return_value = mock_track
    ...
    @pytest.mark.asyncio
    async def test_search_albums_success(self, service, mock_tidal_session, sample_tidal_album):
        """Test successful album search."""
        mock_tidal_session.search.return_value = {'albums': [sample_tidal_album]}
        
        with patch.object(service, '_convert_tidal_album', new_callable=AsyncMock) as mock_convert:
            mock_album = Album(
                id="456",
                title="Test Album",
                artists=[Artist(id="789", name="Test Artist")]
    ...
    @pytest.mark.asyncio
    async def test_search_artists_success(self, service, mock_tidal_session, sample_tidal_artist):
        """Test successful artist search."""
        mock_tidal_session.search.return_value = {'artists': [sample_tidal_artist]}
        
        with patch.object(service, '_convert_tidal_artist', new_callable=AsyncMock) as mock_convert:
    ...
    @pytest.mark.asyncio
    async def test_search_playlists_success(self, service, mock_tidal_session, sample_tidal_playlist):
        """Test successful playlist search."""
        mock_tidal_session.search.return_value = {'playlists': [sample_tidal_playlist]}
        
        with patch.object(service, '_convert_tidal_playlist', new_callable=AsyncMock) as mock_convert:
    ...
    async def test_search_all_success(self, service):
        ...
        with patch.object(service, 'search_tracks', new_callable=AsyncMock, return_value=[mock_track]), \
             patch.object(service, 'search_albums', new_callable=AsyncMock, return_value=[mock_album]), \
             patch.object(service, 'search_artists', new_callable=AsyncMock, return_value=[mock_artist]), \
             patch.object(service, 'search_playlists', new_callable=AsyncMock, return_value=[mock_playlist]):
...
class TestPlaylistManagement:
    ...
    @pytest.mark.asyncio
    async def test_get_playlist_success(self, service, mock_tidal_session, sample_tidal_playlist):
        ...
        with patch.object(service, '_convert_tidal_playlist', new_callable=AsyncMock) as mock_convert:
            ...
            mock_convert.assert_called_once_with(sample_tidal_playlist, include_tracks=True)
    ...
    @pytest.mark.asyncio
    async def test_get_playlist_tracks_success(self, service, mock_tidal_session, sample_tidal_playlist, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            ...
            results = await service.get_playlist_tracks("playlist-uuid-123")
            
            assert len(results) == 1
            assert results[0] == mock_track
    
    @pytest.mark.asyncio
    async def test_create_playlist_success(self, service, mock_tidal_session, sample_tidal_playlist):
        ...
        with patch.object(service, '_convert_tidal_playlist', new_callable=AsyncMock) as mock_convert:
            ...
            result = await service.create_playlist("My New Playlist", "Test description")
    ...
    @pytest.mark.asyncio
    async def test_add_tracks_to_playlist_success(self, service, mock_tidal_session, sample_tidal_playlist, sample_tidal_track):
        ...
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
            result = await service.add_tracks_to_playlist("playlist-uuid-123", ["123456"])
    ...
    async def test_add_tracks_to_playlist_invalid_playlist(self, service):
        ...
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=False), \
            ...
            result = await service.add_tracks_to_playlist("invalid-playlist", ["123456"])
            assert result is False
    
    @pytest.mark.asyncio
    async def test_add_tracks_to_playlist_no_tracks(self, service):
        """Test adding empty track list to playlist."""
        result = await service.add_tracks_to_playlist("playlist-uuid-123", [])
        assert result is False
    
    @pytest.mark.asyncio
    async def test_remove_tracks_from_playlist_success(self, service, mock_tidal_session, sample_tidal_playlist):
        """Test successful track removal from playlist."""
        mock_tidal_session.playlist.return_value = sample_tidal_playlist
        sample_tidal_playlist.remove_by_index.return_value = True
        
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
            result = await service.remove_tracks_from_playlist("playlist-uuid-123", [0, 2, 1])
            
            assert result is True
            # Should be called in reverse order to avoid index shifting
            expected_calls = [2, 1, 0]
            actual_calls = [call[0][0] for call in sample_tidal_playlist.remove_by_index.call_args_list]
            assert actual_calls == expected_calls
    
    @pytest.mark.asyncio
    async def test_delete_playlist_success(self, service, mock_tidal_session, sample_tidal_playlist):
        """Test successful playlist deletion."""
        mock_tidal_session.playlist.return_value = sample_tidal_playlist
        sample_tidal_playlist.delete.return_value = True
        
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
            result = await service.delete_playlist("playlist-uuid-123")
            
            assert result is True
            sample_tidal_playlist.delete.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_get_user_playlists_success(self, service, mock_tidal_session, sample_tidal_playlist):
        ...
        with patch.object(service, '_convert_tidal_playlist', new_callable=AsyncMock) as mock_convert:
            ...
            assert len(results) == 1
            assert results[0] == mock_playlist
            mock_convert.assert_called_once_with(sample_tidal_playlist, include_tracks=False)
...
class TestFavoritesManagement:
    ...
    @pytest.mark.asyncio
    async def test_get_user_favorites_tracks(self, service, mock_tidal_session, sample_tidal_track):
    ...
    @pytest.mark.asyncio
    async def test_add_to_favorites_track_success(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
            ...
            assert result is True
            mock_tidal_session.user.favorites.add_track.assert_called_once_with(sample_tidal_track)
    ...
    @pytest.mark.asyncio
    async def test_remove_from_favorites_success(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
            ...
            assert result is True
            mock_tidal_session.user.favorites.remove_track.assert_called_once_with(sample_tidal_track)
...
class TestRecommendationsAndRadio:
    ...
    @pytest.mark.asyncio
    async def test_get_track_radio_success(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(id="123456", title="Radio Track", artists=[], duration=240)
            mock_convert.return_value = mock_track
    ...
    @pytest.mark.asyncio
    async def test_get_artist_radio_success(self, service, mock_tidal_session, sample_tidal_artist, sample_tidal_track):
    ...
    @pytest.mark.asyncio
    async def test_get_recommended_tracks_from_favorites(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(id="123456", title="Recommended Track", artists=[], duration=240)
            mock_convert.return_value = mock_track
    ...
    @pytest.mark.asyncio
    async def test_get_recommended_tracks_fallback_to_featured(self, service, mock_tidal_session, sample_tidal_track):
        ...
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(id="123456", title="Featured Track", artists=[], duration=240)
            mock_convert.return_value = mock_track
...
class TestDetailedItemRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_get_track_success(self, service, mock_tidal_session, sample_tidal_track):
    ...
    @pytest.mark.asyncio
    async def test_get_album_success(self, service, mock_tidal_session, sample_tidal_album):
        ...
        with patch.object(service, '_convert_tidal_album', new_callable=AsyncMock) as mock_convert:
            ...
            with patch('tidal_mcp.utils.validate_tidal_id', return_value=True):
                ...
                mock_convert.assert_called_once_with(sample_tidal_album, include_tracks=False)
    
    @pytest.mark.asyncio
    async def test_get_artist_success(self, service, mock_tidal_session, sample_tidal_artist):
    ...
    @pytest.mark.asyncio
    async def test_get_album_tracks_success(self, service, mock_tidal_session, sample_tidal_album, sample_tidal_track):
        """Test successful album tracks retrieval."""
        sample_tidal_album.tracks.return_value = [sample_tidal_track]
        mock_tidal_session.album.return_value = sample_tidal_album
        
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock) as mock_convert:
            mock_track = Track(id="123456", title="Album Track", artists=[], duration=240)
            mock_convert.return_value = mock_track
    ...
    @pytest.mark.asyncio
    async def test_get_artist_albums_success(self, service, mock_tidal_session, sample_tidal_artist, sample_tidal_album):
    ...
    @pytest.mark.asyncio
    async def test_get_artist_top_tracks_success(self, service, mock_tidal_session, sample_tidal_artist, sample_tidal_track):
...
class TestConversionMethods:
    ...
    async def test_convert_tidal_playlist_success(self, service, sample_tidal_playlist):
        ...
        result = await service._convert_tidal_playlist(sample_tidal_playlist, include_tracks=False)
        
        assert isinstance(result, Playlist)
        assert result.id == "playlist-uuid-123"
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_conversion_error_handling(self, service, mock_tidal_session, sample_tidal_track):
        """Test handling of conversion errors."""
        mock_tidal_session.search.return_value = {'tracks': [sample_tidal_track]}
        
        with patch.object(service, '_convert_tidal_track', new_callable=AsyncMock, side_effect=Exception("Conversion error")):
            results = await service.search_tracks("test")
