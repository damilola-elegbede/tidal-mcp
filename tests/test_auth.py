...
class TestTidalAuth:
    ...
    async def test_ensure_valid_token_refresh_needed(self, auth):
        ...
        with patch.object(
            auth, "refresh_access_token", new_callable=AsyncMock
        ) as mock_refresh:
...
class TestErrorHandling:
    ...
    def test_get_user_info_exception_handling(self, auth, mock_tidal_session):
        ...
        type(mock_tidal_session).user = PropertyMock(
            side_effect=Exception("User fetch failed")
        )
