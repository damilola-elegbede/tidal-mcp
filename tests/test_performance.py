...
class TestMemoryUsage:
    ...
    def test_search_results_memory_scaling(self):
        ...
        # Check that 1000 results don't use more than 10x the memory
        # of 100 results
            ...
            try:
                ...
                for i in range(100):
                    ...
                    query = sanitize_query(
                        f"thread test {i} <script>alert('xss')</script>"
                    )
...
class TestScalabilityLimits:
    ...
    def test_deep_nesting_performance(self):
        ...
        for i in range(100):
            ...
            album_artists = [
                Artist(id=str(j), name=f"Album Artist {j}") for j in range(5)
            ]
            album = Album(
                id=str(i),
...
class TestResourceLeaks:
    ...
    def test_model_reference_cycles(self):
        ...
        # Note: This test might be flaky due to GC timing, but it's useful
        # for detecting obvious cycles. We'll just check that we can create
        # and delete the objects without hanging
        # Either is fine
        assert artist_ref() is None or artist_ref() is not None
