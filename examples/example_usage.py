    ...
    try:
        ...
        async with TidalService(auth) as service:
            ...
            for track in tracks[:3]:  # Show first 3 results
                logger.info(
                    f"  - {track.title} by {track.artist_names} ({track.formatted_duration})"
                )
            ...
            for album in albums:
                ...
                logger.info(
                    f"  - {album.title} by {artists} ({album.number_of_tracks} tracks)"
                )
            ...
            new_playlist = await service.create_playlist(
                "My MCP Test Playlist", "Created via Tidal MCP server example"
            )
            ...
            if new_playlist:
                ...
                if tracks:
                    ...
                    success = await service.add_tracks_to_playlist(
                        new_playlist.id, track_ids
                    )
                    if success:
    ...
    try:
        ...
        async with TidalService(auth) as service:
            ...
            for query in search_queries:
                ...
                batch_results.append({"query": query, "results": artists})
...
async def example_playlist_management():
    ...
    try:
        async with TidalService(auth) as service:
            ...
            if new_playlist:
                logger.info(
                    f"Created playlist: {new_playlist.title} (ID: {new_playlist.id})"
                )
                ...
                if track_ids_to_add:
                    ...
                    success = await service.add_tracks_to_playlist(
                        new_playlist.id, track_ids_to_add
                    )
                    ...
                    if success:
                        ...
                        if updated_playlist:
                            logger.info(
                                f"Playlist now has {updated_playlist.number_of_tracks} tracks"
                            )
                            logger.info(
                                f"Total duration: {updated_playlist.formatted_duration}"
                            )
