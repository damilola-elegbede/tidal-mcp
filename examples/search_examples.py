...
async def search_tracks_example():
    ...
    # Basic track search
    basic_results = await tidal_search(query="Daft Punk", content_type="tracks", limit=5)
    logger.info(f"Basic Search Results: {len(basic_results.get('results', {}).get('tracks', []))} tracks")
    
    # Paginated search
    paginated_results = await tidal_search(query="Daft Punk", content_type="tracks", limit=3, offset=5)
    logger.info(f"Paginated Search Results: {len(paginated_results.get('results', {}).get('tracks', []))} tracks")
...
async def search_albums_example():
    ...
    album_results = await tidal_search(query="Electronic", content_type="albums", limit=10)
    logger.info(f"Album Search Results: {len(album_results.get('results', {}).get('albums', []))} albums")
...
async def search_artists_example():
    ...
    artist_results = await tidal_search(query="French House", content_type="artists", limit=5)
    logger.info(f"Artist Search Results: {len(artist_results.get('results', {}).get('artists', []))} artists")
