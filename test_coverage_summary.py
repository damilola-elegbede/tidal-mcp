...
def analyze_source_files():
    ...
    for src_file in source_files:
        ...
        try:
            ...
            mcp_tool_lines = [line for line in content.split('\n') if '@mcp.tool' in line]
...
def main():
    ...
    server_test_exists = any("test_server.py" in filename for filename in test_analysis.keys())
    
    print(f"✅ MCP Tools ({len(mcp_tools)} total): {'Covered' if server_test_exists else 'Not Covered'}")
    ...
    additional_tests = {
        'Integration Tests': any('test_integration.py' in f for f in test_analysis.keys()),
        'Error Handling Tests': any('test_error_handling.py' in f for f in test_analysis.keys()),
        'Performance Tests': any('test_performance.py' in f for f in test_analysis.keys()),
    }
    ...
    print(f"  Test to Code Ratio: {total_test_functions / (total_functions + total_mcp_tools + 1):.2f}")
