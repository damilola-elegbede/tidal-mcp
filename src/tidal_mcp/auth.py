...
Supports OAuth2 flow with PKCE and token management for secure API interactions.
"""
...
class TidalAuth:
    ...
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None):
        ...
        Args:
            client_id: Tidal API client ID (optional, uses default if not provided)
            ...
            if self.tidal_session.load_oauth_session(
                ...
                try:
                    ...
                    if user and user.id:
                        ...
                        logger.info(f"Successfully loaded existing session for user {self.user_id}")
                        return True
                except Exception:
    ...
    async def _capture_auth_code(self) -> Optional[str]:
        ...
        try:
            ...
            while auth_code is None and (asyncio.get_event_loop().time() - start_time) < timeout:
    ...
    async def _exchange_code_for_tokens(self, auth_code: str, code_verifier: str) -> bool:
        ...
        try:
            ...
            if self.tidal_session.load_oauth_session(
                ...
                raise TidalAuthError("Failed to initialize Tidal session with tokens")
    ...
    async def _revoke_tokens(self) -> None:
        ...
        try:
            # Tidal doesn't have a public revoke endpoint, so we just invalidate locally
